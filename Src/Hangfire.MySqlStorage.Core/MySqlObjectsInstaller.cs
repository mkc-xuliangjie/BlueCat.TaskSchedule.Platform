...
namespace Hangfire.MySqlStorage.Core
{
    public static class MySqlObjectsInstaller
    {
        ...
        public static void Install(MySqlConnection connection, MySqlStorageOptions options)
        {
            ...
            var script = GetStringResource("Hangfire.MySqlStorage.Core.Install.sql").Replace("<tableprefix>", options.TablePrefix);
            //var script = GetStringResource("Install.sql").Replace("<tableprefix>", options.TablePrefix);
