...
namespace BlueCat.Core.Options
{
    public class ConfigurationManager
    {
        ...
        /// <param name="def"> 缺省值（不存在或不合理时使用） </param>
        /// <returns> 长整数值 </returns>
        public static long GetAppSettingLong(string key, long def = 0L)
        {
        ...
        /// <param name="def"> 缺省值（不存在或不合理时使用） </param>
        /// <returns> 双精数值 </returns>
        public static bool GetAppSettingBool(string key, bool def = true)
        {
            return AppSettings.GetBool(key, def);
        }
