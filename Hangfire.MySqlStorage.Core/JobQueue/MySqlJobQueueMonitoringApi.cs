        ...
        public IEnumerable<string> GetQueues()
        {
            lock (_cacheLock)
            {
                if (_queuesCache.Count == 0 || _cacheUpdated.Add(QueuesCacheTimeout) < DateTime.UtcNow)
                {
                    ...
                    {
                        return connection.Query($"select distinct(Queue) from {_options.TablePrefix}_JobQueue").Select(x => (string)x.Queue).ToList();
                    });
...
select r.JobId from (
  ...
  from "+_options.TablePrefix+ @"_JobQueue jq
...
where r.rankvalue between @start and @end;";
        ...
        public EnqueuedAndFetchedCountDto GetEnqueuedAndFetchedCount(string queue)
        {
            return _storage.UseConnection(connection =>
            {
                var result = 
                    connection.Query<int>(
                        $"select count(Id) from {_options.TablePrefix}_JobQueue where Queue = @queue", new { queue = queue }).Single();
